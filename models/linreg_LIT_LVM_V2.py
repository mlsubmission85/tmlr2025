...
class LinearRegression_LIT_LVM(nn.Module):
    ...
    def fit(self, X_train, X_train_interaction, y_train,X_val = None, X_val_interaction=None, y_val=None, learning_rate=0.001, num_epochs=500, ES_threshold = 20):
        ...
        self.estimated_Zr = nn.Parameter(torch.randn(self.p,self.d, requires_grad=True))
        self.estimated_Zr = torch.nn.Parameter(self.estimated_Zr.to(self.device))
        ...
        if  not self.interaction and not self.ls_penalty: 
            ...
            optimizer = optim.Adam([self.estimated_w, self.estimated_V, self.estimated_Zd, self.estimated_Zr], lr=learning_rate)
        else:
