...
if noise and sparsity:
    file_path = os.path.join(folder_name, f"{model}_p:{p}_d:{d}_lr:{learning_rate}_Noise:{noise}_V_noise:{V_noise}_interaction:{interaction}_Sparsity:{sparsity}_sigma:{sigma}.txt")
elif noise and not sparsity:
    file_path = os.path.join(folder_name, f"{model}_p:{p}_d:{d}_lr:{learning_rate}_Noise:{noise}_V_noise:{V_noise}_interaction:{interaction}_Sparsity:{sparsity}.txt")
elif not noise and sparsity:
    file_path = os.path.join(folder_name, f"{model}_p:{p}_d:{d}_lr:{learning_rate}_Noise:{noise}_interaction:{interaction}_Sparsity:{sparsity}_sigma:{sigma}.txt")
else:
    file_path = os.path.join(folder_name, f"{model}_p:{p}_d:{d}_lr:{learning_rate}_Noise:{noise}_interaction:{interaction}_Sparsity:{sparsity}.txt")
...
for data_split in range(5):
    ...
    model_1 = LogisticRegression_LIT_LVM(V_dim=p ,d=2, estimated_w=estimated_w1,estimated_V=estimated_V1, interaction = interaction, regularization=True, ls_penalty=False)
    ...
    for model in models:
        ...
        if model.d == 2:
            res_2.append(auc)
            if interaction:
                res_w_mse.append(w_mse.detach().cpu().item())
                res_V_mse.append(V_mse.detach().cpu().item())
...
print(f'result_2_w_MSE: {res_w_mse}, mean: {round(np.mean(res_w_mse), 3)}, SE: {round(np.std(res_w_mse, ddof=1) / np.sqrt(len(res_w_mse)), 3)}', file=results_text)
print(f'result_2_V_MSE: {res_V_mse}, mean: {round(np.mean(res_V_mse), 3)}, SE: {round(np.std(res_V_mse, ddof=1) / np.sqrt(len(res_V_mse)), 3)}', file=results_text)
